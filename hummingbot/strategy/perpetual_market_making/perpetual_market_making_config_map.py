...
def validate_price_type(value: str) -> Optional[str]:
    ...
    if price_source != "custom_api":
        valid_values = {"mid_price", "last_price", "last_own_trade_price", "best_bid", "best_ask"}
...
perpetual_market_making_config_map = {
    "strategy": ConfigVar(key="strategy", prompt=None, default="perpetual_market_making"),
    ...
    "market": ConfigVar(
        key="market", prompt=maker_trading_pair_prompt, validator=validate_derivative_trading_pair, prompt_on_new=True
    ),
    "leverage": ConfigVar(
        key="leverage",
        prompt="设置杠杆 " "(Binance Perpetual supports up to 75X for most pairs) >>> ",
        type_str="int",
    ...
    "position_mode": ConfigVar(
        key="position_mode",
        prompt="设置仓位模式 (One-way(单向持仓)/Hedge(双向持仓)) >>> ",
    ...
    "stop_loss_spread": ConfigVar(
        ...
        prompt="您想以与开仓价格的多少价差下止损单？ （输入 1 表示 1%） >>> ",
        type_str="decimal",
        default=Decimal("0"),
    ...
    "order_level_amount": ConfigVar(
        key="order_level_amount",
    ...
    "order_level_spread": ConfigVar(
        key="order_level_spread",
        prompt="Enter the price increments (as percentage) for subsequent " "orders? (Enter 1 to indicate 1%) >>> ",
        required_if=lambda: perpetual_market_making_config_map.get("order_levels").value > 1,
        type_str="decimal",
        validator=lambda v: validate_decimal(v, 0, 100, inclusive=False),
        default=Decimal("1"),
    ...
    "price_source_derivative": ConfigVar(
        key="price_source_derivative",
    ...
    "price_source_custom_api": ConfigVar(
        key="price_source_custom_api",
        prompt="Enter pricing API URL >>> ",
        required_if=lambda: perpetual_market_making_config_map.get("price_source").value == "custom_api",
        type_str="str",
    ),
    "custom_api_update_interval": ConfigVar(
    ...
    "order_override": ConfigVar(
        key="order_override", prompt=None, required_if=lambda: False, default=None, type_str="json"
    ),
}
