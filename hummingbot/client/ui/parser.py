...
def load_parser(hummingbot: "HummingbotApplication", command_tabs) -> [ThrowingArgumentParser, Any]:
    ...
    connect_parser = subparsers.add_parser("connect", help="列出可用的交易所并向其添加 API 密钥")
    ...
    create_parser = subparsers.add_parser("create", help="创建新机器人")
    create_parser.add_argument("--script-config", dest="script_to_config", nargs="?", default=None, help="v2 策略名称")
    create_parser.set_defaults(func=hummingbot.create)

    import_parser = subparsers.add_parser("import", help="通过加载配置文件导入已有机器人")
    ...
    history_parser.add_argument(
        "-v", "--verbose", action="store_true", default=False, dest="verbose", help="显示所有交易"
    )
    history_parser.add_argument(
    ...
    gateway_parser = subparsers.add_parser("gateway", help="网关服务器的帮助程序命令。")
    gateway_subparsers = gateway_parser.add_subparsers()

    gateway_balance_parser = gateway_subparsers.add_parser(
        "balance", help="Display your asset balances and allowances across all connected gateway connectors"
    )
    gateway_balance_parser.set_defaults(func=hummingbot.gateway_balance)

    gateway_config_parser = gateway_subparsers.add_parser("config", help="查看或更新网关配置")
    ...
    gateway_connector_tokens_parser.add_argument(
        "chain_network", nargs="?", default=None, help="Name of chain_network you want to edit reported tokens for"
    )
    gateway_connector_tokens_parser.add_argument(
        "new_tokens",
        nargs="?",
    ...
    previous_strategy_parser = subparsers.add_parser("previous", help="导入上次使用的策略")
    ...
    mqtt_start_parser = mqtt_subparsers.add_parser("start", help="启动 MQTT 桥接器")
    mqtt_start_parser.add_argument("-t", "--timeout", default=30.0, type=float, dest="timeout", help="桥接器连接超时")
    ...
    mqtt_restart_parser = mqtt_subparsers.add_parser("restart", help="重启 MQTT 桥接器")
    mqtt_restart_parser.add_argument("-t", "--timeout", default=30.0, type=float, dest="timeout", help="桥接器连接超时")
